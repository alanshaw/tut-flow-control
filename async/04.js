...
module.exports = function (req, res) {
  var data = {}
  var taskCount = 3 // How to get rid of?
  var errd = false

  userService.getUser(function (er, user) {
    if (er && !errd) { // Repetition
      errd = true // Repetition
      return res.status(500).json({message: "Failed to get user"})
    }
    data.profile = user.profile
    taskCount-- // Repetition
    if (taskCount == 0) res.json(data) // Repetition
  })

  promoService.getPromos(req.body.keyword, function (er, promos) {
    if (er && !errd) { // Repetition
      errd = true // Repetition
      return res.status(500).json({message: "Failed to get promos"})
    }
    data.promos = promos
    taskCount-- // Repetition
    if (taskCount == 0) res.json(data) // Repetition
